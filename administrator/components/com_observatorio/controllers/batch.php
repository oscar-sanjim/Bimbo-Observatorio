...
class ObservatorioControllerBatch extends JControllerForm
{
    ...
    private function processCSVFile($filepPath)
    {
        ...
        try {
            ...
            $columns = array();
            $values = array();
            foreach (Constants::CSVCOLUMNS as $key => $value) {
            ...
            if (($handle = fopen($filepPath, "r")) !== FALSE) {
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    ...
                    $rowsValues = array();
                    foreach (Constants::CSVCOLUMNS as $key => $value) {
                        array_push($rowsValues, $db->quote($data[Constants::CSVCOLUMNS[$key]]));
            ...
            $query->insert($db->quoteName('#__com_observatorio_records'));
    ...
    private function deletePreviousBatches($trimester, $year)
    {
        ...
        $query->delete($db->quoteName('#__com_observatorio_batch'));
